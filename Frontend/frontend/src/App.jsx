...
function App() {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        const response = await fetch("http://15.206.72.115:8000/initialize", {
          method: "POST",
        });
        ...
        setInitError(
          "Failed to connect to backend on http://15.206.72.115:8000",
        );
      }
    };
    initializeApp();
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch("http://15.206.72.115:8000/ask", {
        method: "POST",
